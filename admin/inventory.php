...
<body class="h-[100dvh] overflow-hidden text-gray-800 relative flex bg-[#f8f9fa]">
    ...
    <!-- Mobile Header -->
    <div
        class="md:hidden fixed top-0 left-0 right-0 h-20 bg-white border-b border-gray-100 flex items-center justify-between px-6 z-[60]">
        <h1 class="text-xl font-black uppercase tracking-tighter text-[#0f0f0f]">DecoraTV <span
                class="text-amber-500">Admin</span></h1>
        <button onclick="toggleMobileMenu()"
            class="w-12 h-12 flex items-center justify-center bg-[#0f0f0f] text-white rounded-xl shadow-lg active:scale-95 transition-transform">
    ...
    <div id="mobileOverlay" onclick="toggleMobileMenu()"
        class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[70] hidden opacity-0 transition-opacity duration-300">
    </div>
    ...
    <aside id="sidebarMenu"
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar px-4 pb-10">
            <nav class="space-y-1 mt-2">
                <a href="index.php"
                    class="sidebar-link flex items-center gap-3 px-6 py-4 rounded-xl text-[14px] font-black uppercase tracking-widest">
                ...
                <a href="inventory.php?type=frame"
                    class="sidebar-link <?php echo $type === 'frame' ? 'active' : ''; ?> flex items-center gap-3 px-6 py-4 rounded-xl text-[14px] font-black uppercase tracking-widest">
                    <i class="fa-solid fa-border-all w-5"></i> Frames
                </a>
                <a href="inventory.php?type=liner"
                    class="sidebar-link <?php echo $type === 'liner' ? 'active' : ''; ?> flex items-center gap-3 px-6 py-4 rounded-xl text-[14px] font-black uppercase tracking-widest">
                    <i class="fa-solid fa-maximize w-5"></i> Liners
                </a>
                <a href="inventory.php?type=art"
                    class="sidebar-link <?php echo $type === 'art' ? 'active' : ''; ?> flex items-center gap-3 px-6 py-4 rounded-xl text-[14px] font-black uppercase tracking-widest">
                ...
                <a href="inquiries.php"
                    class="sidebar-link flex items-center gap-3 px-6 py-4 rounded-xl text-[14px] font-black uppercase tracking-widest text-gray-400">
                ...
                <div class="pt-6 border-t border-white/5 mt-6">
                    <a href="logout.php"
                        class="text-gray-500 hover:text-red-400 text-[14px] font-black uppercase tracking-widest transition-colors flex items-center gap-3 px-6 py-4">
    ...
    <main class="flex-1 overflow-y-auto custom-scrollbar mt-20 md:mt-0 p-6 md:p-12">
        <header class="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16">
            <div>
                ...
                <div class="flex items-center gap-2 mt-3">
                    ...
                    <p class="text-[16px] text-gray-500 font-bold uppercase tracking-widest">Add or Remove Items from
                        your Catalog</p>
                </div>
            </div>
            <a href="../" target="_blank"
                class="bg-gray-900 text-white px-10 py-5 rounded-3xl text-[16px] font-black uppercase tracking-widest hover:bg-amber-600 transition-all flex items-center gap-3">
                Simulator
            </a>
        </header>
        ...
        <?php if ($message): ?>
            <div
                class="mb-10 p-6 rounded-2xl text-[16px] font-black uppercase tracking-widest <?php echo strpos($message, 'Error') !== false ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'; ?>">
                <?php echo $message; ?>
            </div>
        <?php endif; ?>
                ...
                <div class="bg-white card p-10 sticky top-10">
                    <div class="flex items-center justify-between mb-10">
                        <h3 class="text-[16px] font-black uppercase tracking-widest text-gray-500">
                            <?php echo $editItem ? 'Edit' : 'Add New'; ?> <?php echo ucfirst($type); ?></h3>
                        <?php if ($editItem): ?>
                            <a href="inventory.php?type=<?php echo $type; ?>"
                                class="text-[12px] font-black uppercase tracking-widest text-amber-600 hover:text-amber-700">Cancel</a>
                        <?php endif; ?>
                    </div>
                    <form method="POST" enctype="multipart/form-data" class="space-y-8">
                        ...
                        <div>
                            <label
                                class="block text-[14px] font-black uppercase tracking-[0.2em] text-gray-400 mb-3 ml-1">Name
                                / Title</label>
                        ...
                        <div>
                            <label
                                class="block text-[14px] font-black uppercase tracking-[0.2em] text-gray-400 mb-3 ml-1">Internal
                                Reference ID</label>
                            <input type="text" name="internal_id" id="internalIdInput"
                                value="<?php echo $editItem ? htmlspecialchars($editItem['internal_id']) : ''; ?>"
                                placeholder="e.g. F123-B" required
                                class="w-full bg-gray-50 border border-gray-100 rounded-2xl px-8 py-5 text-[16px] focus:outline-none focus:border-amber-500/50 transition-all">
                        </div>
                        <?php if ($type === 'art'): ?>
                            <div>
                                <label
                                    class="block text-[14px] font-black uppercase tracking-[0.2em] text-gray-400 mb-3 ml-1">Artist
                                    Name</label>
                                <input type="text" name="artist"
                                    value="<?php echo $editItem ? htmlspecialchars($editItem['artist']) : ''; ?>"
                                    class="w-full bg-gray-50 border border-gray-100 rounded-2xl px-8 py-5 text-[16px] focus:outline-none focus:border-amber-500/50 transition-all">
                            </div>
                        <?php endif; ?>
                        <div>
                            <label
                                class="block text-[14px] font-black uppercase tracking-[0.2em] text-gray-400 mb-3 ml-1">Group
                                / Category</label>
                            <input type="text" name="group_name"
                                value="<?php echo $editItem ? htmlspecialchars($editItem['group_name']) : ''; ?>"
                                placeholder="General"
                                class="w-full bg-gray-50 border border-gray-100 rounded-2xl px-8 py-5 text-[16px] focus:outline-none focus:border-amber-500/50 transition-all">
                        </div>
                        <div>
                                ...
                                if (file) {
                                    ...
                                    if (nameInput || internalIdInput) {
                                        ...
                                        if (internalIdInput && (!isEdit || internalIdInput.value.trim() === '')) {
                                            ...
                                            if (type === 'frame') {
                                                internalIdInput.value = fileName;
                                            } else {
                        ...
                        <button type="submit"
                            class="w-full bg-gray-900 text-white py-6 rounded-2xl font-black uppercase tracking-[0.2em] text-[16px] shadow-xl hover:bg-amber-600 transition-all shadow-gray-200">
                        ...
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-gray-50 text-[11px]">
                                <?php if (empty($items)): ?>
                                    <tr>
                                        <td colspan="4"
                                            class="px-10 py-20 text-center text-gray-500 italic uppercase font-bold tracking-[0.2em]">
                                            The collection is currently empty</td>
                                    </tr>
                                <?php else:
                                        ...
                                        <tr class="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td class="px-10 py-8 text-right">
                                                <div class="flex items-center justify-end gap-2">
                                                    ...
                                                    <a href="?type=<?php echo $type; ?>&delete=<?php echo $item['id']; ?>"
                                                        onclick="return confirm('Archive this item?')"
                                                        class="w-12 h-12 inline-flex items-center justify-center rounded-xl text-gray-300 hover:text-red-500 hover:bg-red-50 transition-all border border-transparent hover:border-red-100">
    ...
    <div id="lightbox-modal" onclick="if(event.target === this) closeLightbox()"
        class="fixed inset-0 bg-black/90 backdrop-blur-sm z-[100] hidden items-center justify-center p-4 md:p-10 cursor-pointer">
        <button onclick="closeLightbox()"
            class="absolute top-10 right-10 text-white/50 hover:text-white transition-colors z-50">
    ...
    <button id="backToTop" onclick="scrollToTop()"
        class="fixed bottom-10 right-10 w-12 h-12 bg-gray-900 text-white rounded-2xl shadow-2xl flex items-center justify-center opacity-0 translate-y-20 pointer-events-none transition-all duration-500 hover:bg-amber-600 z-[90] active:scale-95">
